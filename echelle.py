...
def fix_margins():
    plt.subplots_adjust(left=0.12, right=0.95, bottom=0.14, top=0.90)
...
def overplot(starfile):
    ...
    for i, datafile in enumerate(datafiles):
        ...
        h, plot_position = echelle(starfile, dnu)
        ...
        plt.plot(*plot_position(fl0_obs[0]), 'ro')
        plt.plot(*plot_position(closestfl0_list[i]), 'md')
        plt.plot(*plot_position(fl0), 'bo')
        plt.plot(*plot_position(fl0_obs), 'b', marker='d')
        # for abe, kat in zip(fcorr, llist):
        #    plt.plot(*plot_position(abe), color=color[kat], marker='d')
        plt.plot(*plot_position(f[l == 1]), 'go')
